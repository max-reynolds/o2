...
namespace o2
{
	...
	{
		ConvertString(*this, TString<T>(std::to_string(value).c_str()));
        super::erase(std::basic_string<T>::find_last_not_of('0') + 1, std::string::npos);
        super::erase(std::basic_string<T>::find_last_not_of('.') + 1, std::string::npos);
	}
	...
	TString<T>::operator Vec2F() const
	{
		...
		{
            wchar_t *px = super::c_str(), *py;
			return Vec2F(wcstof(px, &py), wcstof(py, nullptr));
		}
		else
		{
            char *px = super::c_str(), *py;
